...
export function bookingChoiceClass(selected: boolean, className?: string) {
  return cn(
    "cursor-pointer transition-colors rounded-lg border p-4 text-left flex flex-col gap-1 min-h-[4.5rem]",
    selected
...
export const selectionButtonClass = cn(
  "cursor-pointer transition-colors rounded-lg border p-4 text-left flex flex-col gap-1 min-h-[4.5rem] border-border hover:border-ruby/40",
);
